...
exports.createMerchant = async (req, res)=>{
    try {
        ...
        const app_name_id = crypto.randomBytes(4).toString("hex");
        const app_secret_key = crypto.randomBytes(16).toString("hex");

        const existingMerchant = await Merchant.findOne({name});
        if(existingMerchant){
            return res.status(400).json({isSuccess:false, message:"Merchant Is Already Exists!"})
        }
...
exports.getAllMerchants = async (req,res)=>{
    try {
        ...
        return res.status(200).json({isSuccess:true, message:"Merchant Lists Reterived Successfully!", merchants})
    } catch (error) {
        return res.status(500).json({isSuccess:false, message: error.message})
    }
}
...
exports.getMerchantById = async (req,res)=>{
    try {
        ...
        if (!merchant) {
            return res.status(404).json({ isSuccess: false, message: "Merchant Not Found!" });
          }
